...
u8 isinlight(s32 x, s32 y, s32 z)
{
    ...
    while ((light == 0) & (block == 0) & (steps < 150)) {
        ...
        if (light == 0) {
            if (mp[xx / 65536][zz / 65536][yy / 65536][0] > 0) {
                block = 1;
            }
        }
    }
...
void newlight(signed long x, signed long y, signed long z, unsigned long lightdata)
{
    ...
    if (thisframenice) {
        ...
        // flare
        s32 rg = -(FPML(x - camera[0], vcamera[2][0]) + FPML(z - camera[1], vcamera[2][1])
                   + FPML(y - camera[2], vcamera[2][2]));
        s32 rx, ry, dist0, dist;

        if (rg > 0) {
            if (rg < lflare[1]) {
                rx = (FPML(x - camera[0], vcamera[0][0]) + FPML(z - camera[1], vcamera[0][1])
                      + FPML(y - camera[2], vcamera[0][2]));
                ry = -(FPML(x - camera[0], vcamera[1][0]) + FPML(z - camera[1], vcamera[1][1])
                       + FPML(y - camera[2], vcamera[1][2]));
                dist0 = 6556;
