...
void newblendsprite(unsigned char index, unsigned char alpha, signed long xx, signed long yy, signed long zz)
{
    if (blendcount < 63) {
        ...
        blenddata[blendcount][5] = -(FPML(xx - camera[0], vcamera[2][0]) + FPML(yy - camera[1], vcamera[2][1])
        ...
        for (k = 0; k < blendcount; k++) {
            ...
            switch (blenddata[i][0]) {
                ...
                case 1: // fog
                    ...
                    zlBeginQuads();
                    zlTexCoord2x(0, 0);
                    zlVertex3x(x + vv[0][0], z + vv[0][1], y + vv[0][2]);
                    zlTexCoord2x(65536, 0);
                    zlVertex3x(x + vv[1][0], z + vv[1][1], y + vv[1][2]);
                    zlTexCoord2x(65536, 65536);
                    zlVertex3x(x + vv[2][0], z + vv[2][1], y + vv[2][2]);
                    zlTexCoord2x(0, 65536);
                    zlVertex3x(x + vv[3][0], z + vv[3][1], y + vv[3][2]);
                    zlEndQuads();
                    break;
            }
        }
