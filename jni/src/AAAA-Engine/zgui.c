...
void procnoise(void)
{
    ...
    if (thisframenice) {
        if (tension[7] > 0) {
            ...
            zlTexCoord2x(65536 * s + x, y);
            zlVertex3x(65536, 65536, 0);
            zlTexCoord2x(65536 * s + x, 65536 * s + y);
            zlVertex3x(65536, -65536, 0);
            zlTexCoord2x(x, 65536 * s + y);
            zlVertex3x(-65536, -65536, 0);
            zlEndQuads();
            zlBlend(0);
        }
...
void aaagui(void)
{
    ...
    if (mob[mobcontrol][39] > 0) {
        guialarmed = 1;
    }

    if (mob[mobcontrol][40] > 0) {
        guialarmed = 1;
    }
    ...
    if (guialpha > 0) {
        if (thisframenice) {
            ...
            for (k = 0; k < 3; k++) {
                if (k > 2 - mob[mobcontrol][42] | mob[mobcontrol][0] <= 0) {
                    t = 0;
                } else {
                    t = 32768;
                }
                ...
                zlBeginQuads();
                zlTexCoord2x(t + 0, 0);
                zlVertex3x(x - sx, y + sy, 0);
                zlTexCoord2x(t + 32768, 0);
                zlVertex3x(x + sx, y + sy, 0);
                zlTexCoord2x(t + 32768, 65536);
                zlVertex3x(x + sx, y - sy, 0);
                zlTexCoord2x(t + 0, 65536);
                zlVertex3x(x - sx, y - sy, 0);
                zlEndQuads();
                x += l;
            }
...
void lensflare(void)
{
    ...
    if (lflare[0] > 0) {
        ...
        */
        ...
        x = x0;
        y = y0;
        sx = 35000 + f_sin[count * 10 & 4095] / 10;
        sy = (sx / 3) * 4;
        t = 0;
...
void showexitdialog(void)
{
    ...
    if (thisframenice) {
        ...
        zlColor4x(255, 255, 255, 255);
        zlPush();
        zlTranslate(-58000, 2000, 0);
        l_text = sprintf(s_text, "PRESS [SEL] TO EXIT GAME");
...
void autismgui(void)
{
    ...
    if (autismdelay > 400) {
        z = (autismdelay - 400) * 55;
    }

    if (autismdelay < 128) {
        z = -(128 - autismdelay) * 155;
    }
    ...
    if (thisframenice) {
        ...
        zlTexCoord2x(0, 0);
        zlVertex3x(-45536, 45536, 0);
        zlTexCoord2x(65536, 0);
        zlVertex3x(-32768, 32768, 0);
        zlTexCoord2x(65536, 65536);
        zlVertex3x(-32768, -32768, 0);
        zlTexCoord2x(0, 65536);
        zlVertex3x(-45536, 0, 0);
        ...
        zlVertex3x(32768, 32768, 0);
        zlTexCoord2x(65536, 65536);
        zlVertex3x(6768, 45768, 0);
        zlTexCoord2x(0, 65536);
        zlVertex3x(-45768, 45768, 0);
...
void procendlevel(void)
{
    ...
    if (endlevel < 255) {
        if (ltime[0] == 0 & ltime[1] == 0) {
            ...
            if (level >= 2 & level < 10) {
                ...
                if (ltime[0] * 60 + ltime[1] < configdata[16 + i * 2] * 60 + configdata[16 + i * 2 + 1]) {
                    ...
                    if (i < 8) {
                        if (configdata[16 + (i + 1) * 2] == 199) {
                            configdata[16 + (i + 1) * 2] = 99;
                        }
                    }
                }
            }
        ...
        if (playmode == ZPM_AAA) {
            ...
            zlPush();
            zlTranslate(-38000, 00, 0);
            l_text = sprintf(s_text, "%i%i%iM%i%iS", ltime[0] / 100, ltime[0] / 10 % 10, ltime[0] % 10, ltime[1] / 10,
                             ltime[1] % 10);
            zlScale(5000, 9000, 10000);
            zlRenderText();
            zlPop();
    ...
    if (narration == 0 | playmode == ZPM_MOVIE) {
        if (endlevel > 32) {
            ...
            if (endlevel == 32) { // reallyend;
                if (mpheader[22] == 0) {
                    newgamemode = ZGM_MENU;
                } else if (mpheader[22] == 1) {
                    zcoreenabled = 0;
                } else {
                    LoadNewMap(mpheader[22]);
                }
            }
        }
    }
