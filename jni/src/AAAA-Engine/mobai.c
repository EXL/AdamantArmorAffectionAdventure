...
void mobpeace(unsigned char i)
{
    ...
    if (((mob[i][19]) % 128) == 0) {
        ...
        if (zlrand() % 2 == 1) {
            zcplaysound3d(mobsonic[mob[i][10]][8], 4, mob[i][1], mob[i][2], mob[i][3] + 65536);
        }
    }
...
void mobattack(unsigned char i)
{
    ...
    if (((mob[i][19]) % 16) == 0) {
        if (insight) {
            ...
            mob[i][29] = 0;
            mob[i][30] = AI_PEACE;
            zcplaysound3d(mobsonic[mob[i][10]][10], 8, mob[i][1], mob[i][2], mob[i][3] + 65536);
        }
